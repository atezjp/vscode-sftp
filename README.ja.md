...
現在の公開版では、SSH 切断時の後始末を安定化し、`password` の省略時と空文字指定時の扱いも分かりやすく整理しています。
...
```
`sftp.json` の `password` は省略可能です。省略した場合は同期時にパスワードの入力が求められます。`"password": ""` は未設定ではなく、空パスワードを明示指定する扱いになります。
