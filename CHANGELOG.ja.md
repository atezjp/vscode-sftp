...
## 1.16.11 - 2026-03-10
* README 冒頭を整理し、現行公開版の改善点が分かりやすいよう案内文を更新。
* 不要になった issue / pull request 案内文を削除し、Marketplace の短い説明文を VS Code / Cursor / Antigravity の共通設定利用が伝わる表現に調整。

## 1.16.10 - 2026-03-10
* SSH 切断イベントのハンドラ登録を修正し、`close` / `end` が重なっても終了処理が多重実行されにくいよう改善。
* 設定バリデーションで `password: ""` を許可し、空パスワード明示時の扱いが分かるよう日本語ドキュメントを補足。

## 1.16.9 - 2026-03-10
* 別ドメイン公開用として、初心者向けの単体 HTML ガイドを `web/` に追加。
* 対応エディタ間で `.vscode/sftp.json` を共通利用する形に整理し、`ssh2` を同梱して Cursor でも拡張が起動しやすいよう修正。

## 1.16.8 - 2026-03-08
* Cursor / Antigravity でエディタごとの `sftp.json` を優先して読み込むよう修正。
* 対応済みの設定パスだけを設定ファイルとして判定し、保存時の再読み込みが正しく反映されるよう改善。

## 1.16.7 - 2026-03-08
* 日本語の「かんたん使い方ガイド」を追加し、README とドキュメント一覧から辿れるように整理。
* 日本語ドキュメントを VSIX に含めるようにして、インストール後もローカルリンクから参照できるよう改善。
